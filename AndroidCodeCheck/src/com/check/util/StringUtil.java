...
public class StringUtil {
	static final int[] wi = new int[] { 7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9,
			10, 5, 8, 4, 2, 1 };
	static final int[] vi = new int[] { 1, 0, 88, 9, 8, 7, 6, 5, 4, 3, 2 };
	public static final String EMPTY = "";
	private static final Pattern URI_FILE_PATTERN = Pattern
			.compile("\\.(zip|rar|tar|apk|gz|z|exe|dmg|wav|mp3|mpeg|rm|avi|ram|doc|ppt|pdf|xls|xlsx|rtf|tmp|bat|shell|swf)$");
	...
	public static String toDecimalString(int value) {
		return value < 0 ? ""
				: (value == 0 ? "0" : (value % 100 == 0 ? String
						.valueOf(value / 100) : Float
						.toString((float) value / 100.0F)));
	}

	public static String toDecimalString(long value) {
		return value < 0L ? "" : (value == 0L ? "0"
				: (value % 100L == 0L ? String.valueOf(value / 100L) : Double
						.toString((double) value / 100.0D)));
	}
	...
	public static String halfUpToDecimalString(int value) {
		if (value < 0) {
			...
			strBuff.append((new BigDecimal(String
					.valueOf((float) value / 100.0F))).setScale(0, 4));
	...
	public static String formatAirportName(String airportName) {
		return emptyOrNull(airportName) ? ""
				: (airportName.contains("鍥介檯") ? airportName.replace("鍥介檯", "")
						: airportName);
	}
	...
	public static String getFlightOrderState(String code) {
		if (emptyOrNull(code)) {
			...
			return c.equals("c") ? "宸插彇娑�" : (c.equals("p") ? "澶勭悊涓�" : (c
					.equals("r") ? "鍏ㄩ儴閫�绁�" : (c.equals("s") ? "鎴愪氦" : (c
					.equals("t") ? "閮ㄥ垎閫�绁�" : (c.equals("w") ? "鏈鐞�" : (c
					.equals("u") ? "鏈彁浜�" : "鎻愪氦涓�"))))));
		}
	}
	...
	public static byte checkIsBackMoney(String giftType) {
		if (!emptyOrNull(giftType)) {
			if (giftType.equalsIgnoreCase("L")
					|| giftType.equalsIgnoreCase("U")
					|| giftType.equalsIgnoreCase("R")
					|| giftType.equalsIgnoreCase("T")
					|| giftType.equalsIgnoreCase("S")) {
				return (byte) 0;
			}

			if (giftType.equalsIgnoreCase("C")
					|| giftType.equalsIgnoreCase("D")) {
				return (byte) 1;
			}
		}
	...
	public static String shortAirportName(String airportName) {
		if (airportName != null) {
			...
			if (airportName.endsWith("鍥介檯鏈哄満")) {
				return airportName.substring(0, len - 4);
			}

			if (airportName.endsWith("鏈哄満")) {
	...
	public static boolean isConSpeCharacters(String string) {
		return string.replaceAll("[涓�-榫*[a-z]*[A-Z]*\\d*-*_*\\s*", "").length() == 0;
	}

	public static boolean isValidChineseNationality(String nationality) {
		return "".equals(nationality) ? true : "CNTWHKMO".indexOf(nationality
				.toUpperCase()) > -1;
	}

	public static boolean isFlightID(String flightid) {
		Pattern p = Pattern
				.compile("^(CA|CZ|FM|MU|BK|JD|EU|CN|NS|HU|VD|G5|HO|KY|3U|SC|ZH|GS|PN|JR|MF|8L|KN|QF|OS)+[A-Z0-9]{3,4}");
	...
	public static String formatDateString(int year, int month, int day) {
		String value;
		for (value = String.valueOf(year); value.length() < 4; value = "0"
				+ value) {
			;
		}
	...
	public static String replaceStr(String str, String src, String aim) {
		return !emptyOrNull(str) && !emptyOrNull(src) ? (str.contains(src) ? str
				.replace(src, aim) : str)
				: "";
	}
	...
	public static int getSeekBarProgress(int value, int minValue, int midValue,
			int maxValue) {
		return value < 0 ? 0 : (value <= midValue ? 50 * (value - minValue)
				/ (midValue - minValue) : 50 + 50 * (value - midValue)
				/ (maxValue - midValue));
	}
	...
	public static int getSeekBarValue(int progress, int minValue, int midValue,
		...
		if (progress < 0) {
			...
			return (int) ((double) (midValue - minValue) * ((double) progress / 50.0D))
					+ minValue;
		} else {
	...
	public static boolean isValidStr(String inputStr) {
		...
		for (int i = 0; i < length; ++i) {
			if ((charArray[i] < 48 || charArray[i] > 57)
					&& (charArray[i] < 65 || charArray[i] > 122)
					&& charArray[i] != 95) {
				return false;
			}
		}
	...
	public static boolean stringCompare(String firstStr, String secondStr) {
		return emptyOrNull(firstStr) ? false
	...
	public static String dateToString(int year, int month, int day) {
		return year >= 0 && month >= 0 && day >= 0 ? year + "-" + month + "-"
				+ day : "";
	}
	...
	public static String getFlightClassName(String code) {
		return "Y".equalsIgnoreCase(code) ? "缁忔祹鑸�"
				: ("F".equalsIgnoreCase(code) ? "澶寸瓑鑸�" : ("A"
						.equalsIgnoreCase(code) ? "涓嶉檺" : ("C"
						.equalsIgnoreCase(code) ? "鍏姟鑸�" : "")));
	}

	public static String getFlightClassName2(String code) {
		return "Y".equalsIgnoreCase(code) ? "缁忔祹鑸�"
				: ("F".equalsIgnoreCase(code) ? "鍏姟鑸�/澶寸瓑鑸�" : ("A"
						.equalsIgnoreCase(code) ? "涓嶉檺" : ("C"
						.equalsIgnoreCase(code) ? "鍏姟鑸�/澶寸瓑鑸�" : "")));
	}
	...
	public static String getFlightSubClassNameByFlag(int flag) {
		...
		switch (flag) {
		case 1:
			subClassName = "楂樼缁忔祹鑸�";
			break;
		case 2:
			subClassName = "瓒呭�煎ご绛夎埍";
			break;
		case 3:
			subClassName = "璞崕缁忔祹鑸�";
			break;
		case 4:
			subClassName = "鍏姟鑸�";
			break;
	...
	public static String insertSymbolInStrPotion(String srcStr,
		...
		if (emptyOrNull(srcStr)) {
			...
			if (position > srcStr.length()) {
				...
				StringBuffer showStringBuffer = new StringBuffer(
						srcStr.length() + insertStr.length());
	...
	public static int calcTwoDateUnsign(String dateStr1, String dateStr2) {
		...
		try {
			long diff = sd.parse(dateStr2).getTime()
	...
	public static String subString(String content, int length) {
		return content.length() < length ? content : content.substring(0,
				length);
	}
	...
	public static float parseToHalfInteger(float value) {
		if (value > 0.0F) {
			...
			return value > (float) valueInt + 0.5F ? (float) (valueInt + 1)
					: (value > (float) valueInt ? (float) valueInt + 0.5F
							: value);
		} else {
			return 0.0F;
		}
	...
	public static String getFormatCurrency(String currency) {
		return currency != null && currency.length() != 0 ? (!"RMB"
				.equalsIgnoreCase(currency)
				&& !"CNY".equalsIgnoreCase(currency) ? currency : "锟�") : "";
	}
	...
	public static Map<String, String> handleExtensionStrToMap(
			String extensionStr) {
	...
	public static String handleExtensionStrToGetValue(String extensionStr,
			String key) {
	...
	public static String getShortPriceDate(String priceDate) {
		if (emptyOrNull(priceDate)) {
			...
			String[] strArray = priceDate.split("銆�");
			if (null != strArray && strArray.length > 2) {
				StringBuilder sb = new StringBuilder();
				sb.append(strArray[0]).append("銆�").append(strArray[1])
						.append("...");
				return sb.toString();
			} else {
				return priceDate;
	...
	public static String getTalkTime(int time) {
		...
		if (hour == 0) {
			str = String
					.format("%02d:%02d", new Object[] { Integer.valueOf(minu),
							Integer.valueOf(sec) });
		} else {
			str = String.format("%02d:%02d:%02d",
					new Object[] { Integer.valueOf(hour),
							Integer.valueOf(minu), Integer.valueOf(sec) });
		}
	...
	public static String convertDispatchFee(int value) {
		return value == 0 ? "0" : (value % 100 == 0 ? String
				.valueOf(value / 100) : Float.toString((float) value / 100.0F));
	}
	...
	public static boolean isDateTimeEmpty(String dateTime) {
		...
		return emptyOrNull(dateTime) ? true : (dateTime
				.equals("00010101000000") ? true : dateTime.equals("00010101"));
	}
	...
	public static String subStringBetween(String source, String start,
		...
		if (source != null && start != null && end != null) {
			...
			if (indexOf == -1) {
				...
				return indexOf2 != -1 ? source.substring(start.length()
						+ indexOf, indexOf2) : null;
			}
		} else {
			return null;
		}
		...
		if (objArr == null) {
			...
			if (i3 <= 0) {
				...
				StringBuilder stringBuilder = new StringBuilder(
						((objArr[i] == null ? 128 : objArr[i].toString()
